...
const DetailTiketPage = () => {
    ...
    const tiketWismaItems = selectedItem.korektif_wisma_detail;
                ...
                <View key={index} style={{
                    ...
                    <View style={{ marginBottom: 10 }}>
                        <HStack justifyContent="space-between">
                            <Text w={Cons.sw2}>{item.item_wisma.nama}</Text>
                            <Text style={{ fontWeight: 'bold' }}>{formatNumber(item.qty)}x{formatNumber(item.harga_rab_mitra)}</Text>
                        </HStack>
                    </View>
                </View>
        ));
            ...
            <View style={{ flex: 1, justifyContent: 'start', alignItems: 'center', paddingTop: 10, paddingBottom: 40 }}>
                <VStack style={{
                    ...
                    <HStack justifyContent='space-between' style={{ marginTop: 10 }}>
                        <Text>Total RAB</Text>
                        <Text style={{ fontWeight: 'bold', paddingTop: 5, }}>{formatNumber(selectedItem.total_rab_mitra)}</Text>
                    </HStack>
